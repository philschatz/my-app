...
function useOrganizationMembers(): MembershipState {
    ...
    const [state, setState] = useState<MembershipState>({ organization: organization, members: [], invitations: [] })
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            setState({
                ...
                invitations: invites.data.filter((invite) => invite.status === 'pending')  // invites can have status == "revoked"
            })
        });
    }, [!!organization])
...
const InviteUser: React.FC<{ organization: Organization }> = ({ organization }) => {
    const form = useForm({
        mode: 'uncontrolled',
        initialValues: {
            email: '',
    ...
    const onSubmit = form.onSubmit(async (values) => {
        ...
        try {
            await organization?.inviteMember({ emailAddress: values.email, role: 'org:member' }).then(() => {
                notifications.show({
                    title: 'User invited',
    ...
    return (
        <form onSubmit={onSubmit}>
            <Flex direction="row" align="flex-end" gap="lg" justify="center" mt="md">
                <TextInput
                    withAsterisk
                    label="Invite member"
...
const onDeleteMember = async (member?: Membership) => {
    if (!member) {
        return;
    }
...
const onCancelInvite = async (invite: Invitation) => {
    try {
        await invite.revoke()
        notifications.show({
...
const InvitesTable: React.FC<{invitations: Invitation[]}> = ({invitations}) => {
    ...
    return (
        <Table mt="xl">
            <Table.Caption style={{ captionSide: 'top' }} >Invitations</Table.Caption>
            <Table.Thead>
                <Table.Tr>
            ...
            <Table.Tbody>
                {invitations.map((invite) => (
                    <Table.Tr key={invite.id}>
                        ...
                        <Table.Td><IconTrashXFilled onClick={() => onCancelInvite(invite)} cursor="pointer" /></Table.Td>
                    </Table.Tr>
                ))}
            </Table.Tbody>
        </Table>
                ...
                <Table.Tbody>
                    {members.map((member) => (
                        <Table.Tr key={member.id}>
                            <Table.Td><Avatar src={member.publicUserData.imageUrl} /></Table.Td>
                            <Table.Td>{member.publicUserData.firstName}</Table.Td>
                            <Table.Td>{member.publicUserData.lastName}</Table.Td>
                            <Table.Td>
                                <Protect permission="org:sys_memberships:manage">
                                    <IconTrashXFilled onClick={() => onDeleteMember(member)} cursor="pointer" />
                                </Protect>
                            </Table.Td>
                        </Table.Tr>
                    ))}
