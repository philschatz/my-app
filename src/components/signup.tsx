...
const EmailVerificationStep = () => {
    ...
    return (
        <Stack>
            <Text>
                We have emailed you a code, please enter that code here to
                finish signing up, thanks!
            </Text>
            <form
                onSubmit={verifyForm.onSubmit((values) => handleVerify(values))}
            >
                <TextInput
                    withAsterisk
                    label="Code"
    ...
    const onSubmit = async (values: SignUpFormValues) => {
        ...
        try {
            ...
            const emailError = err.errors?.find(
                (error: any) => error.meta?.paramName === "email_address",
            );
            if (emailError) {
